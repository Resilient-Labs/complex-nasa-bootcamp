...
fetch(`https://data.nasa.gov/resource/gvk9-iz74.json`)
  ...
  .then((data) => {
    ...
    //Centers are sorted alphabetically
...
*/
    ...
    arr2.forEach((item) => {
      ...
      //removing zips longer than 5 digits
...
function getTemp(zip) {
  fetch(
    `https://api.weatherbit.io/v2.0/current?key=${apiKey}&units=i&postal_code=${zip}&country=US`
  )
    .then((res) => res.json()) // parse response as JSON
    .then((info) => {
      console.log(info);
      document.querySelector(
        "#city"
      ).innerText = `${info.data[0]["city_name"]}, ${info.data[0]["state_code"]}`;
      document.querySelector(
        "#description"
      ).innerText = `${info.data[0].weather.description}`;
      document.querySelector(
        "#farenheit"
      ).innerText = `${info.data[0].temp} °F`;
      //   document.querySelector("#precip").innerText = `${info.data[0].precip} %`;
      //   document.querySelector("#uv").innerText = `${Math.round(
      //     info.data[0].uv
      //   )}`;
      //   document.querySelector(
      //     "#humidity"
      //   ).innerText = `Humidity: ${info.data[0].rh} %`;
      //   document.querySelector(
      //     "#pressure"
      //   ).innerText = `Pressure: ${info.data[0].pres}°`;
      //   document.querySelector(
      //     "#windSpeed"
      //   ).innerText = `Wind: ${info.data[0]["wind_spd"]} mph`;
    })
