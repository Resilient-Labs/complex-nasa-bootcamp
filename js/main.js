...
document.querySelector('#facilitiesBtn').addEventListener('click', getNasaFacilities)
...
function getNasaFacilities() {
    ...
    const table = document.querySelector('#results').appendChild(document.createElement('table'))
    ...
    .then(facilityData => {
        for (let i = 0; i <= facilityData.length; i++) {
            ...
            .then(weatherData => {
                const facilityWeather = Math.round((weatherData.main.temp - 273.15) * (9 / 5) + 32)
